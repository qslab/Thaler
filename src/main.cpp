...
unsigned int nStakeMinAge = 60 * 60 * 1; // 1 hours
...
int nCoinbaseMaturity = 10; // 10 blocks for coins to mature
...
const string strMessageMagic = "Thaler Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
	...
	else if (pindexBest->nHeight+1 > 2 && pindexBest->nHeight+1 < 28658 ) 
    {
        nSubsidy = 0 * COIN;
    }
...
static const int64_t nTargetSpacingWorkMax = 1 * nStakeTargetSpacing;
//
...
// Thaler: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Thaler", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "2014. It's only the beginning.";
        CTransaction txNew;
        txNew.nTime = 1423149605;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0xfe1109d5493152f4051c9f5d944c3a98baaab4acb4159824aa4d346256a35e6f"));
        block.print();
